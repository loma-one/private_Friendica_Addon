...
class GuardianPanel
{
    public function getAuditContent(): string
    {
        ...
        $filteredUsers = [];
        foreach ($users as $u) {
            // 1. Filter: Nur Wartende (falls Checkbox aktiv)
            ...
            // --- Anzeige-Entscheidung (Kein Score 0 außer bei Suche) ---
            $show = false;
            if (!empty($search)) {
            ...
            if ($show) {
                // Präzise Status-Logik (Blockwall entfernt)
                $u['status_text'] = 'Aktiv'; $u['status_class'] = 'success';
                if ($u['pending']) {
                    $u['status_text'] = 'Wartend'; $u['status_class'] = 'warning';
                } elseif ($u['account_removed']) {
                    $u['status_text'] = 'Gelöscht'; $u['status_class'] = 'danger';
                } elseif ($u['blocked']) {
                    $u['status_text'] = 'Gesperrt'; $u['status_class'] = 'danger';
                } elseif ($u['account_expired']) {
                    $u['status_text'] = 'Inaktiv'; $u['status_class'] = 'default';
                }
